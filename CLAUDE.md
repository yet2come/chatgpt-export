...
`chat-single-export.js` 冒頭の doc コメントに `vX.Y` ごとの修正点を箇条書きする慣習がある（現在 v7.17）。動作を変える修正を入れるときは:

1. 冒頭の doc コメントに新しいバージョン見出しを足し、変更点を箇条書きで残す。
2. IIFE 末尾の `console.log('   v7.17: …')` を新バージョン文言に更新する。

`chat-bulk-export.js` は single とは独立してバージョン番号を持つ（現在 v8.1）。同じ規則で IIFE 末尾の `console.log('   v8.1: …')` を更新する。
