...
`chat-bulk-export.js` は DOM 経路を持たないため、conversation API と file API の cooldown を分けて扱う。conversation 側は 60 / 90 / 120 / 180 秒の段階 cooldown を使い、通算 5 回の throttle で会話失敗ではなく一時停止として manifest / queue を保存してレジュームに委ねる。file 側は 60 秒固定で同じ asset を再試行する。404 は期限切れ・存在しない asset 参照であり cooldown 起点にしない。
...
- conversation / file の 429 / 503 は別 cooldown として扱う。conversation 側は段階 cooldown + 通算 5 回で一時停止、file 側は 60 秒固定で同じ asset を再試行する。`OPTIONS.maxBatchPauseMs` は kind 別累積と総累積の両方に効く。
...
2. IIFE 末尾の `console.log('   v0.7.20: …')` を新バージョン文言に更新する。

`chat-bulk-export.js` は single とは独立してバージョン番号を持つ（現在 v0.8.15）。同じ規則で IIFE 末尾の `console.log('   v0.8.15: …')` を更新する。
