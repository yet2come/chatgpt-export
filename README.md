...
| `maxBatchPauseMs` | `900000` | クールダウン累積待機の上限（ミリ秒）。超えるとバッチを一時停止し、再実行で resume。v0.8.13 以降、conversation / file の cooldown は分離され、conversation 429/503 は通算閾値で早めに一時停止する。v0.8.15 以降、5 件分の成功蓄積で通算カウントを 1 段階減衰し、cooldown ladder は 60 / 90 / 120 / 180 / 240 / 300 秒の 6 段階 |
...
- conversation API と file API の 429 / 503 は別 cooldown として扱います。conversation API の throttle が減衰しきれず 6 段階 ladder を超えた場合、または累積待機が `maxBatchPauseMs` を超える場合は、manifest / queue を保存して自動一時停止します。10〜30 分空けて同じフォルダで resume してください。
